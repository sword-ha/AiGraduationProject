...
cv_service.py
-------------
Bridges the FastAPI layer with the existing cv_generator pipeline.
Accepts a validated Pydantic schema and returns a CVGenerateResponse.
"""
...
from schemas.cv_schema import ATSBreakdown, CVGenerateRequest, CVGenerateResponse

logger = logging.getLogger(__name__)

# Point at the existing cv_generator project
...
class CVGeneratorService:
    ...
    def _build_cv_dict(self, req: CVGenerateRequest) -> Dict[str, Any]:
        """Convert Pydantic request into the internal cv_data dict format."""
    ...
    def _render_cv_text(self, cv_data: Dict[str, Any]) -> str:
        """Render CV dict to a plain-text ATS-friendly string."""
        lines = []
    ...
    async def generate(self, req: CVGenerateRequest) -> CVGenerateResponse:
        ...
        logger.info(
            f"CV generated for '{req.personal.name}' — ATS score={score_report['total_score']}"
        )
        ...
        return CVGenerateResponse(
            ...
            pdf_url=f"/api/v1/download-cv/{pdf_filename}",
        )
